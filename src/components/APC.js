...
export default class APC extends Chart {
    ...
    draw(){
        ...
        const xplot = scaleLinear()
            .domain([0, this.anchor[0]+1])
            .range([0, this.width])
        
        
        const yplot = scaleLinear()
            .domain([0, this.anchor[1]+1])
            .range([this.height, 0])
        ...
        const linegen = d => {
            ...
            let coordinates = pairs.map((l)=> {
                let x = normalize[l[0]](d[l[0]]) + this.anchor[0]
                let y = normalize[l[1]](d[l[1]]) + this.anchor[1]
                return [[xplot(this.anchor[0]), yplot(this.anchor[1])],[xplot(x), yplot(y)]]
            })

            return line()(coordinates.flat())
        }
